...
class Unit
{
    ...
    {
        $data = \InterNative\Punic\Data::get('units', $locale);
        $precision = null;
        if (is_int($width)) {
            $precision = $width;
            $width = 'short';
        ...
        if (is_null($rules)) {
            ...
            throw new \InterNative\Punic\Exception\ValueNotInList($unit, $units);
        }
        $pluralRule = \InterNative\Punic\Plural::getRule($number, $locale);
        ...
        return sprintf($rules[$pluralRule], \InterNative\Punic\Number::format($number, $precision, $locale));
    }
}
