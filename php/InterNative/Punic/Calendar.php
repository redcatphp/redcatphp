...
class Calendar
{
     ...
     * @throws \InterEthnic\Punic\Exception\BadArgumentType Throws an exception if $value is not empty and can't be converted to a <code>\DateTime</code> instance or if $toTimezone is not empty and is not valid.
     * @example
     * Convert a Unix timestamp to a \DateTime instance with the current time zone:<br>
     * \InterEthnic\Punic\Calendar::toDateTime(1409648286);<br>
     * <br>
     * Convert a Unix timestamp to a \DateTime instance with a specific time zone:<br>
     * \InterEthnic\Punic\Calendar::toDateTime(1409648286, 'Europe/Rome');<br>
     * \InterEthnic\Punic\Calendar::toDateTime(1409648286, new \DateTimeZone('Europe/Rome'));<br>
     * <br>
     * Convert a string to a \DateTime instance with the current time zone:<br>
     * \InterEthnic\Punic\Calendar::toDateTime('2014-03-07 13:30');<br>
     * <br>
     * Convert a string to a \DateTime instance with a specific time zone:<br>
     * \InterEthnic\Punic\Calendar::toDateTime('2014-03-07 13:30', 'Europe/Rome');<br>
     ...
     * @param string $locale = '' The locale to use. If empty we'll use the default locale set with {@link \InterEthnic\Punic\Data::setDefaultLocale()}.
     * @return string Returns an empty string if $value is empty, the name of the era otherwise.
     * @throws \InterEthnic\Punic\Exception\BadArgumentType Throws a BadArgumentType exception if $value is not valid.
     * @throws \InterEthnic\Punic\Exception\ValueNotInList Throws a ValueNotInList exception if $width is not valid.
     * @throws \InterEthnic\Punic\Exception Throws a generic exception in case of other problems (for instance if you specify an invalid locale).
     */
    public static function getEraName($value, $width = 'abbreviated', $locale = '')
    {
        ...
        if ((!empty($value)) || ($value === 0) || ($value === '0')) {
            ...
            $data = \InterEthnic\Punic\Data::get('calendar', $locale);
            $data = $data['eras'];
     ...
     * @throws \InterEthnic\Punic\Exception Throws a generic exception in case of problems (for instance if you specify an invalid locale).
     */
    public static function getTimezoneNameNoLocationSpecific($value, $width = 'long', $kind = '', $locale = '')
    {
        ...
        if (!empty($value)) {
            ...
            if (strlen($receivedPhpName)) {
                $metazoneCode = '';
                $data = \InterEthnic\Punic\Data::getGeneric('metaZones');
                ...
                if (strlen($metazoneCode)) {
                    $data = \InterEthnic\Punic\Data::get('timeZoneNames', $locale);
     ...
     * @param string $locale = '' The locale to use. If empty we'll use the default locale set in \InterEthnic\Punic\Data
    ...
    {
        $result = '';
        $locale = empty($locale) ? \InterEthnic\Punic\Data::getDefaultLocale() : $locale;
        if (!empty($value)) {
            $receivedPhpName = '';
            if (is_string($value)) {
                $receivedPhpName = $value;
            ...
            if (strlen($receivedPhpName)) {
                $phpNames = static::getTimezonesAliases($receivedPhpName);
                $timeZoneNames = \InterEthnic\Punic\Data::get('timeZoneNames', $locale);
                foreach ($phpNames as $phpName) {
    ...
    public static function getFirstWeekday($locale = '')
    {
        ...
        if (!array_key_exists($locale, $cache)) {
            $result = 0;
            $data = \InterEthnic\Punic\Data::getGeneric('weekData');
            $i = \InterEthnic\Punic\Data::getTerritoryNode($data['firstDay'], $locale);
            if (is_int($i)) {
                $result = $i;
            }
            $cache[$locale] = $result;
    ...
    public static function describeInterval($dateEnd, $dateStart = null, $maxParts = 2, $width = 'short', $locale = '')
    {
        ...
        $parts = array();
        $data = \InterEthnic\Punic\Data::get('dateFields', $locale);
        if ($dateEndUTC->getTimestamp() == $dateStartUTC->getTimestamp()) {
            ...
            if (($mostFar < $maxDistance) && ($diff->y > 0)) {
                $parts[] = \InterEthnic\Punic\Unit::format($diff->y, 'duration/year', $width, $locale);
                $mostFar = 0;
            } elseif (!empty($parts)) {
                $mostFar++;
            }
            if (($mostFar < $maxDistance) && ($diff->m > 0)) {
                $parts[] = \InterEthnic\Punic\Unit::format($diff->m, 'duration/month', $width, $locale);
                $mostFar = 0;
            } elseif (!empty($parts)) {
                $mostFar++;
            }
            if (($mostFar < $maxDistance) && ($diff->d > 0)) {
                $parts[] = \InterEthnic\Punic\Unit::format($diff->d, 'duration/day', $width, $locale);
                $mostFar = 0;
            } elseif (!empty($parts)) {
                $mostFar++;
            }
            if (($mostFar < $maxDistance) && ($diff->h > 0)) {
                $parts[] = \InterEthnic\Punic\Unit::format($diff->h, 'duration/hour', $width, $locale);
                $mostFar = 0;
            } elseif (!empty($parts)) {
                $mostFar++;
            }
            if (($mostFar < $maxDistance) && ($diff->i > 0)) {
                $parts[] = \InterEthnic\Punic\Unit::format($diff->i, 'duration/minute', $width, $locale);
                $mostFar = 0;
            } elseif (!empty($parts)) {
                $mostFar++;
            }
            if (empty($parts) || ($diff->s > 0)) {
                $parts[] = \InterEthnic\Punic\Unit::format($diff->s, 'duration/second', $width, $locale);
            }
        ...
        switch ($width) {
            case 'narrow':
            case 'short':
                $joined = \InterEthnic\Punic\Misc::joinUnits($parts, $width, $locale);
                break;
            default:
                $joined = \InterEthnic\Punic\Misc::join($parts, $locale);
                break;
        }
    ...
    public static function format($value, $format, $locale = '')
    {
        ...
        if (!empty($value)) {
            ...
            $cacheKey = empty($locale) ? \InterEthnic\Punic\Data::getDefaultLocale() : $locale;
    ...
    public static function getDateRelativeName($datetime, $ucFirst = false, $locale = '')
    {
        ...
        if (array_key_exists('day', $data)) {
            ...
            if (array_key_exists($key, $data)) {
                $result = $data[$key];
                if ($ucFirst) {
                    $result = \InterEthnic\Punic\Misc::fixCase($result, 'titlecase-firstword');
                }
            }
        }
