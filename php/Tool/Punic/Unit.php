...
class Unit
{
    ...
    {
        $data = \Surikat\Tool\Punic\Data::get('units', $locale);
        $precision = null;
        if (is_int($width)) {
            $precision = $width;
            $width = 'short';
        ...
        if (is_null($rules)) {
            ...
            throw new \Surikat\Tool\Punic\Exception\ValueNotInList($unit, $units);
        }
        $pluralRule = \Surikat\Tool\Punic\Plural::getRule($number, $locale);
        ...
        return sprintf($rules[$pluralRule], \Surikat\Tool\Punic\Number::format($number, $precision, $locale));
    }
}
