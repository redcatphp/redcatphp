...
namespace Unit\DiLoader;
class Json {
	private function getComponent($input) {
		if (is_array($input)) foreach ($input as &$value) $value = $this->getComponent($value);
		
		if (is_object($input) && isset($input->instance)) return new \Unit\DiInstance($input->instance);
		else if (is_object($input) && isset($input->call)) return new \Unit\DiInstance([new Callback($input->call), 'run']);
		else return $input;
	}

	public function load($json, \Unit\DiContainer $dice = null) {
		if ($dice === null) $dice = new \Unit\DiContainer;
