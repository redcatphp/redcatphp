...
namespace Database\RedBeanPHP\Repository;

use Database\RedBeanPHP\OODBBean as OODBBean;
use Database\RedBeanPHP\Observable as Observable;
use Database\RedBeanPHP\Adapter\DBAdapter as DBAdapter;
use Database\RedBeanPHP\BeanHelper\FacadeBeanHelper as FacadeBeanHelper;
...
class Fluid extends Repository
{
	...
	public function dispense( $type, $number = 1, $alwaysReturnArray = FALSE )
	{
		$beans = [];
		for ( $i = 0; $i < $number; $i++ ) {
			$OODBBEAN = defined( 'REDBEAN_OODBBEAN_CLASS' ) ? REDBEAN_OODBBEAN_CLASS : 'Database\RedBeanPHP\OODBBean';
