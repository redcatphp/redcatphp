...
namespace RedBase\RedBeanPHP\Repository;

use RedBase\RedBeanPHP\OODBBean as OODBBean;
use RedBase\RedBeanPHP\Observable as Observable;
use RedBase\RedBeanPHP\Adapter\DBAdapter as DBAdapter;
use RedBase\RedBeanPHP\BeanHelper\FacadeBeanHelper as FacadeBeanHelper;
...
class Fluid extends Repository
{
	...
	private function modifySchema( OODBBean $bean, $property, $value )
	{
		...
		$columnNoQ = $this->writer->safeColumn( $property, TRUE );
	...
	public function dispense( $type, $number = 1, $alwaysReturnArray = FALSE )
	{
		$beans = [];
		for ( $i = 0; $i < $number; $i++ ) {
			$OODBBEAN = defined( 'REDBEAN_OODBBEAN_CLASS' ) ? REDBEAN_OODBBEAN_CLASS : 'RedBase\RedBeanPHP\OODBBean';
