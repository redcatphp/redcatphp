...
namespace RedBase\RedBeanPHP\Driver;

use RedBase\RedBeanPHP\Database;
use RedBase\SqlFormatter;
use RedBase\RedBeanPHP\Driver as Driver;
...
use RedBase\RedBeanPHP\PDOCompatible as PDOCompatible;
use RedBase\RedBeanPHP\Cursor\PDOCursor as PDOCursor;
...
class RPDO implements Driver
{
	...
	*/
	...
	{
		$this->connect();
		if($this->DB->devQuery||$this->DB->devSpeed)
		...
		try {
			...
			if($this->DB->devQuery||$this->DB->devStructure||$this->DB->devError){
				if(!($this->DB->devSpeed||$this->DB->devQuery))
					$this->debugger()->logOpen();
