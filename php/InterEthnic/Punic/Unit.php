...
class Unit
{
    ...
    {
        $data = \InterEthnic\Punic\Data::get('units', $locale);
        $precision = null;
        if (is_int($width)) {
            $precision = $width;
            $width = 'short';
        ...
        if (is_null($rules)) {
            ...
            throw new \InterEthnic\Punic\Exception\ValueNotInList($unit, $units);
        }
        $pluralRule = \InterEthnic\Punic\Plural::getRule($number, $locale);
        ...
        return sprintf($rules[$pluralRule], \InterEthnic\Punic\Number::format($number, $precision, $locale));
    }
}
