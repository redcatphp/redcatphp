...
class Unit
{
    ...
    {
        $data = \I18n\Punic\Data::get('units', $locale);
        $precision = null;
        if (is_int($width)) {
            $precision = $width;
            $width = 'short';
        ...
        if (is_null($rules)) {
            ...
            throw new \I18n\Punic\Exception\ValueNotInList($unit, $units);
        }
        $pluralRule = \I18n\Punic\Plural::getRule($number, $locale);
        ...
        return sprintf($rules[$pluralRule], \I18n\Punic\Number::format($number, $precision, $locale));
    }
}
