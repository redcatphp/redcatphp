...
class PostgreSQL extends \surikat\model\RedBeanPHP\QueryWriter\PostgreSQL {
	...
	function addColumnFulltext($table, $col, $cols=null){
		//$this->addColumn($table,$col,$this->code('tsvector'));
		if($cols){
			$agg = $this->agg;
			$aggc = $this->aggCaster;
			$sep = $this->separator;
			$cc = $this->concatenator;
			...
			foreach($cols as $select){
				...
				switch($relation){
					default:
					case '<':
						$columns[] = Query::autoWrapCol($q.$table.$q.'.'.$q.$col.$q,$table,$col);
						$gb = $q.$table.$q.'.'.$q.'id'.$q;
						if(!in_array($gb,$groupBy))
							$groupBy[] = $gb;
					break;
					case '>':
						$columns[] = "{$agg}(COALESCE(".Query::autoWrapCol("{$q}{$table}{$q}.{$q}{$col}{$q}",$table,$col)."{$aggc},''{$aggc}) {$sep} {$cc})";
					break;
					case '<>':
						$columns[] = "{$agg}(".Query::autoWrapCol("{$q}{$table}{$q}.{$q}{$col}{$q}",$table,$col)."{$aggc} {$sep} {$cc})";
					break;
				}
			}
