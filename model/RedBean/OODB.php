...
class OODB extends Observable
{
	...
	private function unboxIfNeeded( $bean )
	{
		...
		if ( !( $bean instanceof OODBBean ) ) {
			throw new Exception_Security( 'OODB Store requires a bean, got: ' . gettype( $bean ) );
		}
	...
	private function getTypeFromCast( $cast )
	{
		if ( $cast == 'string' ) {
			...
			throw new Exception_Security( 'Invalid Cast' );
		}
	...
	private function processSharedAdditions( $bean, $sharedAdditions )
	{
		foreach ( $sharedAdditions as $addition ) {
			if ( $addition instanceof OODBBean ) {
				...
				throw new Exception_Security( 'Array may only contain OODBBeans' );
			}
		}
	}
	...
	public function __construct( QueryWriter $writer )
	{
		...
		$this->beanhelper = new BeanHelper_Facade();
	}
	 ...
	 * throw an exception: Exception_Security.
	 *
	 * @param OODBBean $bean the bean that needs to be checked
	 *
	 * @return void
	 *
	 * @throws Exception_Security $exception
	 */
	public function check( OODBBean $bean )
	{
		//Is all meta information present?
		if ( !isset( $bean->id ) ) {
			throw new Exception_Security( 'Bean has incomplete Meta Information id ' );
		}
		if ( !( $bean->getMeta( 'type' ) ) ) {
			throw new Exception_Security( 'Bean has incomplete Meta Information II' );
		}
		//Pattern of allowed characters
		$pattern = '/[^a-z0-9_]/i';
		//Does the type contain invalid characters?
		if ( preg_match( $pattern, $bean->getMeta( 'type' ) ) ) {
			throw new Exception_Security( 'Bean Type is invalid' );
		}
		...
		foreach ( $bean as $prop => $value ) {
			if (
				...
				throw new Exception_Security( "Invalid Bean value: property $prop" );
			} else if (
				strlen( $prop ) < 1
				|| preg_match( $pattern, $prop )
			) {
				throw new Exception_Security( "Invalid Bean property: property $prop" );
			}
		}
	}
	 ...
	 *
	 * @deprecated Use QueryWriter_AQueryWriter::typeExists() instead.
	 *
	...
	public function getAssociationManager()
	{
		if ( !isset( $this->assocManager ) ) {
			throw new Exception_Security( 'No association manager available.' );
		}
