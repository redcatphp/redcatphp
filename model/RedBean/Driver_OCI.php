 ...
 */
class Driver_OCI implements Driver
{
	/**
	 * @var string
	 */
	...
	public static function getInstance( $dsn, $user, $pass )
	{
		if ( is_null( self::$instance ) ) {
			self::$instance = new Driver_OCI( $dsn, $user, $pass );
		}
	 ...
	 * Examples:
	 *    $driver = new Driver_OCI($dsn, $user, $password);
	 *    $driver = new Driver_OCI($existingConnection);
	 *
	...
	protected function runQuery( $sql, $aValues )
	{
		...
		try {
			...
			$transformedException = new Exception_SQL( $pdoException->getMessage(), 0 );
	...
	public function GetAssocRow($sql, $bindings = array()) {
		throw new Exception_OCI('Not implemented.');
	}
	...
	private function doBinding( $sql, $aValues = array() )
	{
		...
		if ( !$result ) {
			...
			$x = new Exception_SQL( $error['message'] . ':' . $error['sqltext'], 0 );
	...
	public function setDebugMode( $tf, $logger = NULL )
	{
		$this->connect();
		$this->debug = (bool) $tf;
		if ( $this->debug and !$logger ) $logger = new Logger_Default();
