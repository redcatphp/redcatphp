...
class Finder
{
	...
	public function findByPath( $bean, $steps )
	{
		...
		if ( $numberOfSteps % 2 ) {
			throw new Exception_Security( 'Invalid path: needs 1 more element.' );
		}
		...
		for ( $i = 0; $i < $numberOfSteps; $i += 2 ) {
			...
			if ( $steps[$i] === '' ) {
				throw new Exception_Security( 'Cannot access list.' );
			}
			...
			if ( !isset( $list[$steps[$i + 1]] ) ) {
				throw new Exception_Security( 'Cannot access bean.' );
			}
