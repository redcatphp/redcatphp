...
class Facade
{
	...
	/**
	 * @var AssociationManager_ExtAssociationManager
	 */
	...
	{
		$tl = new Plugin_TimeLine($filename);
	...
	public static function transaction( $callback )
	{
		if ( !is_callable( $callback ) ) {
			throw new Exception_Security( 'R::transaction needs a valid callback.' );
		}
	...
	public static function addDatabase( $key, $dsn, $user = NULL, $pass = NULL, $frozen = FALSE, $autoSetEncoding = TRUE )
	{
		if ( isset( self::$toolboxes[$key] ) ) {
			throw new Exception_Security( 'A database has already be specified for this key.' );
		}
	...
	public static function debug( $tf = TRUE, $logger = NULL )
	{
		...
		if ( !isset( self::$adapter ) ) {
			throw new Exception_Security( 'Use R::setup() first.' );
		}
	...
	*/
	...
	public static function dispense( $type, $num = 1 )
	{
		if ( !preg_match( '/^[a-z0-9]+$/', $type ) && self::$strictType ) {
			throw new Exception_Security( 'Invalid type: ' . $type );
		}
	...
	public static function configureFacadeWithToolbox( ToolBox $tb )
	{
		...
		self::$extAssocManager    = new AssociationManager_ExtAssociationManager( self::$toolbox );
    ...
    *
	...
	{
		QueryWriter_AQueryWriter::renameAssociation( $from, $to );
	}
	...
	public static function ext( $pluginName, $callable )
	{
		if ( !ctype_alnum( $pluginName ) ) {
			throw new Exception( 'Plugin name may only contain alphanumeric characters.' );
		}
	...
	public static function __callStatic( $pluginName, $params )
	{
		...
		if ( !isset( self::$plugins[$pluginName] ) ) {
			throw new Exception( 'Plugin \''.$pluginName.'\' does not exist, add this plugin using: R::ext(\''.$pluginName.'\')' );
		}
