 ...
 */
class QueryWriter_Oracle extends QueryWriter_AQueryWriter implements QueryWriter
{
	/**
	 * Adapter
	 *
	...
	public function __construct( Adapter $a )
	{
		...
		$this->typeno_sqltype = array(
			QueryWriter_Oracle::C_DATATYPE_BOOL              => 'NUMBER(1,0)',
			QueryWriter_Oracle::C_DATATYPE_UINT8             => 'NUMBER(3,0)',
			QueryWriter_Oracle::C_DATATYPE_UINT32            => 'NUMBER(11,0)',
			QueryWriter_Oracle::C_DATATYPE_DOUBLE            => 'FLOAT',
			QueryWriter_Oracle::C_DATATYPE_TEXT8             => 'NVARCHAR2(255)',
			QueryWriter_Oracle::C_DATATYPE_TEXT16            => 'NVARCHAR2(2000)',
			QueryWriter_Oracle::C_DATATYPE_TEXT32            => 'CLOB',
			QueryWriter_Oracle::C_DATATYPE_SPECIAL_DATE      => 'DATE',
			QueryWriter_Oracle::C_DATATYPE_SPECIAL_TIMESTAMP => 'TIMESTAMP(6)' );
	...
	protected function constrain( $table, $table1, $table2, $property1, $property2 )
	{
		try {
			...
			if ( $this->code( $columns[$property1] ) !== QueryWriter_Oracle::C_DATATYPE_UINT32 ) {
				$this->widenColumn( $table, $property1, QueryWriter_Oracle::C_DATATYPE_UINT32 );
			}

			if ( $this->code( $columns[$property2] ) !== QueryWriter_Oracle::C_DATATYPE_UINT32 ) {
				$this->widenColumn( $table, $property2, QueryWriter_Oracle::C_DATATYPE_UINT32 );
			}
	...
	{
		$stateMap = array(
			Driver_OCI::OCI_NO_SUCH_TABLE                  => QueryWriter::C_SQLSTATE_NO_SUCH_TABLE,
			Driver_OCI::OCI_NO_SUCH_COLUMN                 => QueryWriter::C_SQLSTATE_NO_SUCH_COLUMN,
			Driver_OCI::OCI_INTEGRITY_CONSTRAINT_VIOLATION => QueryWriter::C_SQLSTATE_INTEGRITY_CONSTRAINT_VIOLATION
		);
	...
	public function scanType( $value, $flagSpecial = FALSE )
	{
		...
		if ( is_null( $value ) ) {
			return QueryWriter_Oracle::C_DATATYPE_BOOL;
		}

		if ( $flagSpecial ) {
			if ( preg_match( '/^\d{4}\-\d\d-\d\d(\s\d\d:\d\d(:\d\d)?)?$/', $value ) ) {
				return QueryWriter_Oracle::C_DATATYPE_SPECIAL_DATE;
			}
			if ( preg_match( '/^\d{4}\-\d\d-\d\d\s\d\d:\d\d:\d\d.\d\d$/', $value ) ) {
				return QueryWriter_Oracle::C_DATATYPE_SPECIAL_TIMESTAMP;
			}
		}
		...
		if ( !$this->startsWithZeros( $value ) ) {
			...
			if ( is_numeric( $value ) && ( floor( $value ) == $value ) && $value >= 0 && $value <= 255 ) {
				return QueryWriter_Oracle::C_DATATYPE_UINT8;
			}

			if ( is_numeric( $value ) && ( floor( $value ) == $value ) && $value >= 0 && $value <= 4294967295 ) {
				return QueryWriter_Oracle::C_DATATYPE_UINT32;
			}

			if ( is_numeric( $value ) ) {
				return QueryWriter_Oracle::C_DATATYPE_DOUBLE;
			}
		}

		if ( strlen( $value ) <= 255 ) {
			return QueryWriter_Oracle::C_DATATYPE_TEXT8;
		}

		if ( strlen( $value ) <= 2000 ) {
			return QueryWriter_Oracle::C_DATATYPE_TEXT16;
		}

		return QueryWriter_Oracle::C_DATATYPE_TEXT32;
	}
