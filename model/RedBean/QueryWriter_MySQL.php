 ...
 */
class QueryWriter_MySQL extends QueryWriter_AQueryWriter implements QueryWriter
{
	...
	protected function constrain( $table, $table1, $table2, $property1, $property2 )
	{
		try {
			...
			if ( $this->code( $columns[$property1] ) !== QueryWriter_MySQL::C_DATATYPE_UINT32 ) {
				$this->widenColumn( $table, $property1, QueryWriter_MySQL::C_DATATYPE_UINT32 );
			}

			if ( $this->code( $columns[$property2] ) !== QueryWriter_MySQL::C_DATATYPE_UINT32 ) {
				$this->widenColumn( $table, $property2, QueryWriter_MySQL::C_DATATYPE_UINT32 );
			}
	...
	public function __construct( Adapter $adapter )
	{
		$this->typeno_sqltype = array(
			QueryWriter_MySQL::C_DATATYPE_BOOL             => ' TINYINT(1) UNSIGNED ',
			QueryWriter_MySQL::C_DATATYPE_UINT8            => ' TINYINT(3) UNSIGNED ',
			QueryWriter_MySQL::C_DATATYPE_UINT32           => ' INT(11) UNSIGNED ',
			QueryWriter_MySQL::C_DATATYPE_DOUBLE           => ' DOUBLE ',
			QueryWriter_MySQL::C_DATATYPE_TEXT8            => ' VARCHAR(255) ',
			QueryWriter_MySQL::C_DATATYPE_TEXT16           => ' TEXT ',
			QueryWriter_MySQL::C_DATATYPE_TEXT32           => ' LONGTEXT ',
			QueryWriter_MySQL::C_DATATYPE_SPECIAL_DATE     => ' DATE ',
			QueryWriter_MySQL::C_DATATYPE_SPECIAL_DATETIME => ' DATETIME ',
			QueryWriter_MySQL::C_DATATYPE_SPECIAL_POINT    => ' POINT ',
		);
	...
	public function scanType( $value, $flagSpecial = FALSE )
	{
		...
		if ( is_null( $value ) ) return QueryWriter_MySQL::C_DATATYPE_BOOL;

		if ( $flagSpecial ) {
			if ( preg_match( '/^\d{4}\-\d\d-\d\d$/', $value ) ) {
				return QueryWriter_MySQL::C_DATATYPE_SPECIAL_DATE;
			}
			if ( preg_match( '/^\d{4}\-\d\d-\d\d\s\d\d:\d\d:\d\d$/', $value ) ) {
				return QueryWriter_MySQL::C_DATATYPE_SPECIAL_DATETIME;
			}
		}
		...
		if ( !$this->startsWithZeros( $value ) ) {
			if ( $value === TRUE || $value === FALSE || $value === '1' || $value === '' ) {
				return QueryWriter_MySQL::C_DATATYPE_BOOL;
			}

			if ( is_numeric( $value ) && ( floor( $value ) == $value ) && $value >= 0 && $value <= 255 ) {
				return QueryWriter_MySQL::C_DATATYPE_UINT8;
			}

			if ( is_numeric( $value ) && ( floor( $value ) == $value ) && $value >= 0 && $value <= 4294967295 ) {
				return QueryWriter_MySQL::C_DATATYPE_UINT32;
			}

			if ( is_numeric( $value ) ) {
				return QueryWriter_MySQL::C_DATATYPE_DOUBLE;
			}
		}

		if ( mb_strlen( $value, 'UTF-8' ) <= 255 ) {
			return QueryWriter_MySQL::C_DATATYPE_TEXT8;
		}

		if ( mb_strlen( $value, 'UTF-8' ) <= 65535 ) {
			return QueryWriter_MySQL::C_DATATYPE_TEXT16;
		}

		return QueryWriter_MySQL::C_DATATYPE_TEXT32;
	}
