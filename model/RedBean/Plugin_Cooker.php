 ...
 */
class Plugin_Cooker implements Plugin
{
	/**
	 * @var boolean
	 */
	...
	private function loadBean( &$array, $filterEmpty )
	{
		...
		if ( isset( $array['id'] ) ) { // Do we need to load the bean?
			if ( self::$loadBeans ) {
				...
				throw new Exception_Security( 'Attempt to load a bean in Cooker. Use enableBeanLoading to override but please read security notices first.' );
			}
		} else {
	...
	private function loadList( &$array, $filterEmpty )
	{
		...
		foreach ( $array as $key => $value ) {
			...
			if ( !( $listBean instanceof OODBBean ) ) {
				throw new Exception_Security( 'Expected bean but got :' . gettype( $listBean ) );
			}
	...
	public function graph( $array, $filterEmpty = FALSE )
	{
		if ( is_array( $array ) && isset( $array['type'] ) ) {
			...
			throw new Exception_Security( 'Expected array but got :' . gettype( $array ) );
		}
	}
