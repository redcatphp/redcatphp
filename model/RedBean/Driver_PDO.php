 ...
 */
class Driver_PDO implements Driver
{
	/**
	 * @var string
	 */
	...
	protected function bindParams( $statement, $bindings )
	{
		foreach ( $bindings as $key => &$value ) {
			if ( is_integer( $key ) ) {
				...
				} elseif ( !$this->flagUseStringOnlyBinding && QueryWriter_AQueryWriter::canBeTreatedAsInt( $value ) && $value < 2147483648 ) {
	...
	protected function runQuery( $sql, $bindings, $options = array() )
	{
		...
		try {
			...
			$exception = new Exception_SQL( $err, 0 );
	...
	*/
	 ...
	 * Examples:
	 *    $driver = new Driver_PDO($dsn, $user, $password);
	 *    $driver = new Driver_PDO($existingConnection);
	 *
	...
	public function connect()
   {
   	...
   	try {
   		...
   		$this->pdo = new \PDO(
			...
			if(\surikat\control::devHas(\surikat\control::dev_model))
