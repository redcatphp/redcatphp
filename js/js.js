...
(function(w,d){	
	...
	var ts;
	var addNoCache = function(fileName){
		if(!ts)
			ts = (new Date().getTime()).toString();
		if(fileName.indexOf('_t=')<0&&fileName.indexOf('://')<0)
	...
	$css = function(fileName, media){
		...
		if(indexOf(loadedCSS,fileName)<0){
			...
			if(!exist){
				...
				if(media)
					style.media =  media;
				style.href = css.dev?addNoCache(fileName):fileName;
	...
	var xDomLoader = function(){
		...
		for(var k in els){
			...
			if(!is)
				is = els[k].tagName;
			var u = (obj.xdom===true?'':obj.xdom)+is.toLowerCase();
			if(indexOf(src,u)<0)
				src.push(u);
		}
		$js(src,true);
