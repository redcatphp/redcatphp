...
/* Converts string date to date array by using date patterns.
 ...
 or dateparser::parse("2008-4-24 20:28:34",[,"Y-m-d H:i:s"])
 
singleton + cleaning + end timezone (experimental) by surikat
*/
class dateparser{	
	//API
	static function parse($date,$format='Y-m-d H:i:s',$timezone=null){
		return self::instance($timezone)->ParseDate($format,$date,$timezone!==null);
	}
	static function format($date,$format='Y-m-d H:i:s',$new_format,$timezone=null){
		return self::instance($timezone)->Format($new_format,$format,$date);
	}
	private static $__singleton;
	static function instance($timezone=null){
		if(!isset(self::$__singleton))
			self::$__singleton = new dateparser;
		return self::$__singleton->__init($timezone);
	}
	private function __init($timezone=null){
		$this->date = null;
		$this->patrVal = null;
